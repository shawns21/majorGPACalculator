...
def calculateFunc(pdf, schoolChoice):
        ...
        if schoolChoice == 'John Jay College':
            creditsAndGrades = JohnJaySearch()
        elif schoolChoice == 'Medgar Evers College':
            creditsAndGrades = MedgarEversSearch()
        elif schoolChoice == 'Staten Island College':
            creditsAndGrades = StatenIslandSearch()
        elif schoolChoice == 'Brooklyn College':
            creditsAndGrades = BrooklynCollegeSearch()
        else:
            return("NO CS CLASSES FOUND")
...
if __name__ == '__main__':
    ...
    try:
        ...
        schools_data = {'John Jay College', 'Medgar Evers College', 'Staten Island College', 'Brooklyn College'}

        def on_upload_button_click():
            file_path = filedialog.askopenfilename()
            if file_path:
                result = calculateFunc(file_path, selected_school)
                if isinstance(result, float):
                    result_label.config(text=f"Major GPA: {result}")
                else:
                    result_label.config(text=result)

        def on_school_button_click(school):
            global selected_school
            selected_school = school
            result_label.config(text="")  # Clear previous result when changing schools
            print(f"Selected school: {selected_school}")
            on_upload_button_click()  # Automatically trigger file upload after school selection

        root = tk.Tk()
        root.title("School Selection and File Upload")

        main_label = tk.Label(root, text="Select your school:")
        main_label.pack(pady=10)

        for school in schools_data:
            school_button = tk.Button(root, text=school, command=lambda s=school: on_school_button_click(s))
    ...
    # Keep the executable open for 10 seconds
    time.sleep(10)